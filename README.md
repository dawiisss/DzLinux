...
### AppImage (Portable)
1. Download `DzLinux-1.4.6.AppImage`.
2. Make it executable:
   ```bash
   chmod +x DzLinux-1.4.6.AppImage
   ```
3. Run or double-click to launch.

### Portable Archive (tar.gz)
1. Download `dzlinux-1.4.6.tar.gz`.
2. Extract to your games directory:
   ```bash
   tar -xzf dzlinux-1.4.6.tar.gz -C ~/Games/
   ```
3. Run the binary:
   ```bash
   cd ~/Games/DzLinux
...
1. Download `dzlinux_1.4.6_amd64.deb`.
2. Install via terminal:
   ```bash
   sudo dpkg -i dzlinux_1.4.6_amd64.deb
   sudo apt install -f
   ```

### RPM Package (rpm) - Fedora, RHEL, openSUSE
1. Download `dzlinux-1.4.6.x86_64.rpm`.
2. Install via terminal:
   ```bash
   sudo rpm -i dzlinux-1.4.6.x86_64.rpm
   ```
